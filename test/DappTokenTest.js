...
contract('DappToken', function (accounts) {
  ...
  it('allocates the total supply on deployment', function () {
    return DappToken.deployed().then(function (instance) {
      ...
      assert.equal(
        ...
        1000000,
        "it allocates the initial supply to the admin's balance");
    });
  });
  ...
  it('transfers token ownership', function () {
    return DappToken.deployed().then(function (instance) {
      ...
      // Test the require statement by transferring something larger than the sender's balance.
      ...
      assert.equal(receipt.logs[0].event, 'Transfer', 'should be a "Tranfer" event');
      assert.equal(receipt.logs[0].args._from, accounts[0], 'logs the account the tokens are transferred from');
      assert.equal(receipt.logs[0].args._to, accounts[1], 'logs the account the tokens are transferred to');
      assert.equal(receipt.logs[0].args._value, 250000, 'logs the transfer amount');
      return tokenInstance.balanceOf(accounts[1]);
    }).then(function (balance) {
      assert.equal(balance.toNumber(), 250000, 'adds the amount to the reveicing account');
      return tokenInstance.balanceOf(accounts[0]);
    }).then(function (balance) {
  ...
  it('handles delegated token transfers', function () {
    return DappToken.deployed().then(function (instance) {
      ...
      assert.equal(balance.toNumber(), 10, 'adds the amount from the spending account');
